...
func (l *ClusterUpgradeLab) SolutionSteps() []SolutionStep {
	return []SolutionStep{
		...
		{Description: "Upgrade control plane", Command: "sudo kubeadm upgrade apply v1.29.0"},
		{Description: "Drain node", Command: "kubectl drain <node-name> --ignore-daemonsets --delete-emptydir-data"},
		{Description: "Upgrade kubelet", Command: "sudo apt-mark unhold kubelet && sudo apt-get update && sudo apt-get install -y kubelet=1.29.0-1.1 && sudo apt-mark hold kubelet"},
